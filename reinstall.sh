...
EOF
    ...
    download_and_extract_udeb() {
        ...
        if ! [ -f $udeb_list ]; then
            curl -L http://$deb_hostname/$distro/dists/$codename/main/debian-installer/binary-$basearch_alt/Packages.gz |
                zcat | grep 'Filename:' | awk '{print $2}' >$udeb_list
        fi

        # 下载 udeb
        curl -Lo $tmp/tmp.udeb http://$deb_hostname/$distro/"$(grep /$package $udeb_list)"
